...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --report)                    REPORT_ONLY=1; DRY_RUN=1; shift ;;
...
# ============================================================================================
MODULE_NAMES="DeltaKeychain DeltaTokens DeltaOffice DeltaOneDrive DeltaBrowser DeltaSSO DeltaWorkAccount AppCache OutlookCache AppSession TokenStoreAll KeychainAll BrowserAll OneDriveAll OutlookProfileAll CompanyPortalAll"
...
module_scope() {
    case "$1" in
        ...
        AppSession|TokenStoreAll|KeychainAll|BrowserAll)                                             echo SignsOut ;;
...
module_desc() {
    case "$1" in
      ...
      AppSession)        echo "Teams and Outlook webview session storage. Both sign in again - no account removed. THIS is what makes Teams re-prompt." ;;
...
PRESET_FULL="$PRESET_SAFE AppSession TokenStoreAll BrowserAll"
...
file_names_target() {
    ...
    case "$(basename "$f")" in
        *"$(lc "$OLD_TENANT_ID")"*) return 0 ;;
    esac
    local g
    for g in ${DELTA_IDS:-}; do
        case "$(lc "$(basename "$f")")" in *"$g"*) return 0 ;; esac
        grep -qai -- "$g" "$f" 2>/dev/null && return 0
    done
    case "$f" in
        *.plist)
...
#  Microsoft keys much of this material by GUID rather than by address: a keychain item named
#  com.microsoft.oneauth.<accountId>.<tenantId>.<objectId> only carries the domain in some of
#  its records. So before deciding anything, harvest every GUID that appears ALONGSIDE the
#  target domain or the old tenant id, then discard any of those GUIDs that ALSO appear
#  alongside another tenant. What remains identifies the old account and nothing else.
#  Without that last exclusion this would match Microsoft's shared first-party client ids and
#  take the new tenant's tokens with it.
# ============================================================================================
DELTA_IDS=""
harvest_delta_ids() {
    local home="$1" u="$2" uid="$3"
    local d1; d1=$(printf '%s' "$TARGET_DOMAINS" | awk '{print $1}')
    local hay="" seen_delta="" seen_other="" g line
...
$home/Library/Application Support/OneDrive/settings"
    local r f
...
$(find "$r" -type f -size -4m 2>/dev/null)
EOF_HF
    done <<EOF_HR
$roots
EOF_HR
    # file names first - the _hrd and account files carry the ids in the name
    while IFS= read -r f; do
        [ -n "$f" ] || continue
        local base; base=$(basename "$f")
        local ids; ids=$(printf '%s' "$base" | grep -oiE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | sort -u)
        [ -n "$ids" ] || continue
        if printf '%s' "$base" | grep -qi -e "$d1" -e "$OLD_TENANT_ID"; then seen_delta="$seen_delta
$ids"
        elif [ -n "$NEW_TENANT_ID" ] && printf '%s' "$base" | grep -qi -- "$NEW_TENANT_ID"; then seen_other="$seen_other
$ids"
        fi
        if grep -qai -e "$d1" -e "$OLD_TENANT_ID" "$f" 2>/dev/null; then
            seen_delta="$seen_delta
$(grep -oaiE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' "$f" 2>/dev/null | sort -u)"
        fi
...
$hay
EOF_HH
    # keychain item names carry the same triplet
    if [ -n "$uid" ]; then
        local inv; inv=$(kc_inventory "$u" "$uid" "$home/Library/Keychains/login.keychain-db")
        while IFS= read -r line; do
            [ -n "$line" ] || continue
            local ids; ids=$(printf '%s' "$line" | grep -oiE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | sort -u)
            [ -n "$ids" ] || continue
            if printf '%s' "$line" | grep -qi -e "$d1" -e "$OLD_TENANT_ID"; then seen_delta="$seen_delta
$ids"
            elif text_names_other_tenant "$line"; then seen_other="$seen_other
$ids"; fi
        done <<EOF_KI
$inv
EOF_KI
    ...
    seen_delta=$(printf '%s\n' "$seen_delta" | tr 'A-F' 'a-f' | awk 'NF' | sort -u)
    seen_other=$(printf '%s\n' "$seen_other" | tr 'A-F' 'a-f' | awk 'NF' | sort -u)
    DELTA_IDS=""
    while IFS= read -r g; do
        [ -n "$g" ] || continue
        [ "$g" = "$(lc "$OLD_TENANT_ID")" ] && continue          # already matched directly
        printf '%s\n' "$seen_other" | grep -qx -- "$g" && continue   # shared with another tenant
        DELTA_IDS="$DELTA_IDS $g"
    done <<EOF_SD
$seen_delta
EOF_SD

    if [ -n "$(printf '%s' "$DELTA_IDS" | tr -d ' ')" ]; then
        log INFO "Identifiers that appear only alongside the old tenant:"
        for g in $DELTA_IDS; do log INFO "   $g"; done
        log INFO "Material keyed by these is treated as the old account's."
    else
        log INFO "No old-tenant-only identifier found beyond the tenant id itself."
    fi
    return 0
}
...
$home/Library/Containers/com.microsoft.OneDrive-mac/Data/Library/Application Support/OneDrive/settings"
    ...
    while IFS= read -r base; do
        [ -d "$base" ] || continue
        # settings/BusinessN AND settings/ListSync/Business_<id> - v3.0/3.1 only globbed the
        # first level, so the ListSync binding survived and VERIFY flagged it. Found in the
        # field log from Mac/lisabronson, 2026-09-23.
        for b in $(find "$base" -maxdepth 3 -type d -name 'Business*' 2>/dev/null); do
            [ -d "$b" ] || continue
...
mod_app_cache() {
    ...
    local ct="$home/Library/Containers" gc="$home/Library/Group Containers" appid ss d
    ...
    # New Teams (2.x). v3.0 only cleared ~/Library/Caches/com.microsoft.teams, which is the
    # CLASSIC path and is usually absent on a current Mac - so Teams looked untouched. The
    # real 2.x scratch lives under the container and the group container, in the Edge WebView
    # cache directories. Local Storage and IndexedDB are deliberately NOT here: those hold the
    # session, and clearing them signs every account out. That is the AppSession module.
    for d in "$ct/com.microsoft.teams2/Data/Library/Caches" \
             "$ct/com.microsoft.teams2/Data/Library/Application Support/Microsoft/MSTeams/EBWebView/Default/Cache" \
             "$ct/com.microsoft.teams2/Data/Library/Application Support/Microsoft/MSTeams/EBWebView/Default/Code Cache" \
             "$ct/com.microsoft.teams2/Data/Library/Application Support/Microsoft/MSTeams/EBWebView/Default/GPUCache" \
             "$ct/com.microsoft.teams2/Data/Library/Application Support/Microsoft/MSTeams/EBWebView/GrShaderCache" \
             "$gc/UBF8T346G9.com.microsoft.teams/Library/Caches" \
             "$gc/UBF8T346G9.com.microsoft.teams/tmp" ; do
        [ -d "$d" ] && clear_folder "$d" "$home" "Teams 2.x - ${d##*/}"
    done
    # Classic Teams, for the Macs that still have it.
    for d in "$home/Library/Application Support/Microsoft/Teams/Cache" \
             ...
             "$home/Library/Caches/com.microsoft.teams" ; do
        [ -d "$d" ] && clear_folder "$d" "$home" "Classic Teams - ${d##*/}"
    done
    ...
    clear_folder "$home/Library/Caches/com.microsoft.Word" "$home" "Word cache"
    clear_folder "$home/Library/Caches/com.microsoft.Excel" "$home" "Excel cache"
...
# ============================================================================================
#  MODULE: AppSession                                                              SIGNS OUT
#  The webview storage where Teams and Outlook keep their signed-in session. Clearing it
#  forces a fresh sign-in for EVERY account in those apps - no account is removed. This is
#  what "make Teams ask me to sign in again" actually needs; AppCache alone will not do it.
# ============================================================================================
mod_app_session() {
    local home="$1" u="$2"
    section "APP SESSION - $u  (Teams and Outlook sign in again, all accounts)"
    local ct="$home/Library/Containers" gc="$home/Library/Group Containers" d f
    local tw="$ct/com.microsoft.teams2/Data/Library/Application Support/Microsoft/MSTeams/EBWebView/Default"
    for d in "$tw/Local Storage" "$tw/Session Storage" "$tw/IndexedDB" "$tw/Service Worker" \
             "$gc/UBF8T346G9.com.microsoft.teams/Library/Application Support/Microsoft/MSTeams" \
             "$home/Library/Application Support/Microsoft/Teams/Local Storage" \
             "$home/Library/Application Support/Microsoft/Teams/IndexedDB" \
             "$home/Library/Application Support/Microsoft/Teams/Session Storage" ; do
        [ -d "$d" ] && clear_folder "$d" "$home" "Teams session - ${d##*/}"
    done
    for f in "$tw/Network/Cookies" "$tw/Cookies" \
             "$home/Library/Application Support/Microsoft/Teams/Cookies" ; do
        [ -f "$f" ] && remove_path "$f" "$home" "Teams cookies - $(basename "$f")"
    done
    local od="$ct/com.microsoft.Outlook/Data/Library"
    for d in "$od/WebKit" "$od/Caches" "$od/Cookies" ; do
        [ -d "$d" ] && clear_folder "$d" "$home" "Outlook session - ${d##*/}"
    done
    remove_path "$home/Library/HTTPStorages/com.microsoft.Outlook" "$home" "Outlook HTTP storage"
    remove_path "$home/Library/HTTPStorages/com.microsoft.teams2"  "$home" "Teams HTTP storage"
    log KEEP "No account is removed and no mail is touched - both apps simply sign in again."
    return 0
}
...
# ============================================================================================
#  REPORT  -  read-only. Answers "why did the delta pass find nothing" with evidence.
#  For every store the delta modules look at, this prints whether it exists, how many files
#  it holds, and which of those files actually contain the target domain as readable text.
#  If a store is full but nothing matches, the identifiers are not stored as plain text there
#  and the content gate cannot see them - which is worth knowing before deleting anything.
# ============================================================================================
report_store() {
    # report_store <label> <path>
    local label="$1" pth="$2" n sz hits
    if [ ! -e "$pth" ]; then printf '  %-46s %s\n' "$label" "ABSENT"; return 0; fi
    n=$(find "$pth" -type f 2>/dev/null | wc -l | tr -d ' ')
    sz=$(du -sh "$pth" 2>/dev/null | awk '{print $1}')
    hits=$(grep -rail -- "$(printf '%s' "$TARGET_DOMAINS" | awk '{print $1}')" "$pth" 2>/dev/null | head -20)
    local hn=0
    [ -n "$hits" ] && hn=$(printf '%s\n' "$hits" | grep -c .)
    printf '  %-46s %6s files  %6s   delta-matching files: %s\n' "$label" "$n" "${sz:-?}" "$hn"
    if [ -n "$hits" ]; then
        printf '%s\n' "$hits" | while IFS= read -r h; do [ -n "$h" ] && printf '        HIT  %s\n' "$h"; done
    fi
    return 0
}
mod_report() {
    local home="$1" u="$2" uid="$3"
    section "REPORT - $u   (read-only, nothing is changed)"
    local gc="$home/Library/Group Containers" ct="$home/Library/Containers"
    log INFO "Stores the delta modules inspect:"
    report_store "OneAuth token store"          "$gc/UBF8T346G9.com.microsoft.oneauth"
    report_store "Office Identity"              "$gc/UBF8T346G9.Office/Identity"
    report_store "Office Licenses"              "$gc/UBF8T346G9.Office/Licenses"
    report_store "Office group container (all)" "$gc/UBF8T346G9.Office"
    report_store "OneDrive settings"            "$home/Library/Application Support/OneDrive/settings"
    report_store "Teams 2.x group container"    "$gc/UBF8T346G9.com.microsoft.teams"
    report_store "Teams 2.x container"          "$ct/com.microsoft.teams2"
    report_store "Classic Teams"                "$home/Library/Application Support/Microsoft/Teams"
    report_store "Outlook container"            "$ct/com.microsoft.Outlook"
    report_store "Company Portal support"       "$home/Library/Application Support/com.microsoft.CompanyPortalMac"
    report_store "HTTPStorages"                 "$home/Library/HTTPStorages"

    log INFO ""
    log INFO "Every address found anywhere in the Office and Teams stores:"
    local addrs
    addrs=$(grep -rhoaE '[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}' \
                "$gc/UBF8T346G9.com.microsoft.oneauth" "$gc/UBF8T346G9.Office" \
                "$gc/UBF8T346G9.com.microsoft.teams" "$ct/com.microsoft.teams2" \
                "$home/Library/Application Support/OneDrive/settings" 2>/dev/null \
            | sort | uniq -c | sort -rn | head -25)
    if [ -n "$addrs" ]; then
        printf '%s\n' "$addrs" | while IFS= read -r a; do [ -n "$a" ] && log INFO "   $a"; done
    else
        log WARN "   None. No address is stored as readable text in any of those stores -"
        log WARN "   they are binary or encrypted, so a content match cannot find the account."
    fi

    if [ -n "$uid" ]; then
        log INFO ""
        log INFO "Keychain items naming the old tenant:"
        local kc="$home/Library/Keychains/login.keychain-db" inv
        inv=$(kc_inventory "$u" "$uid" "$kc")
        if [ -z "$inv" ]; then
            log WARN "   Keychain inventory unreadable - not the signed-in session, or locked."
        else
            log INFO "   items visible: $(printf '%s\n' "$inv" | grep -c .)"
            printf '%s\n' "$inv" | grep -i -- "$(printf '%s' "$TARGET_DOMAINS" | awk '{print $1}')" | while IFS= read -r l; do
                [ -n "$l" ] && log INFO "   HIT  $l"
            done
        fi
        if [ -x /usr/bin/app-sso ]; then
            log INFO ""
            log INFO "Enterprise SSO extension:"
            run_as_user "$u" "$uid" /usr/bin/app-sso -l 2>&1 | while IFS= read -r l; do
                [ -n "$l" ] && log INFO "   $l"
            done
        fi
    else
        log WARN "Keychain and SSO not inspected - $u is not the signed-in user."
    fi

    log INFO ""
    log INFO "Browser profiles and their signed-in addresses:"
    local rel base pdir mails
    for rel in "Google/Chrome" "Microsoft Edge" "BraveSoftware/Brave-Browser"; do
        base="$home/Library/Application Support/$rel"
        [ -d "$base" ] || continue
        for pdir in "$base/Default" "$base"/Profile\ *; do
            [ -f "$pdir/Preferences" ] || continue
            mails=$(grep -oaE '"email":"[^"]+"' "$pdir/Preferences" 2>/dev/null | sed 's/"email":"//; s/"$//' | sort -u | tr '\n' ' ')
            log INFO "   $rel / $(basename "$pdir"): ${mails:-none}"
        done
    done
    return 0
}
...
while IFS= read -r U; do
    ...
    if [ "$REPORT_ONLY" = "1" ]; then
        mod_report "$HOMEDIR" "$U" "$([ "$IS_CONSOLE" = "1" ] && echo "$UID_N" || echo "")"
        PROCESSED_USERS=$((PROCESSED_USERS + 1))
        continue
    fi

    DELTA_IDS=""
    if has_module DeltaTokens || has_module DeltaOffice || has_module DeltaOneDrive || has_module DeltaKeychain; then
        section "IDENTIFIER CORRELATION - $U"
        harvest_delta_ids "$HOMEDIR" "$U" "$([ "$IS_CONSOLE" = "1" ] && echo "$UID_N" || echo "")"
    fi
    ...
    has_module AppSession     && mod_app_session     "$HOMEDIR" "$U"
