...
param(
    ...
    # ---------------------------------------------------------------------------------------
    #  DIAGNOSTIC / TROUBLESHOOTING SWITCHES  (added for the fleet rollout investigation)
    # ---------------------------------------------------------------------------------------

    # Collect a full environment snapshot, capture raw output of every external tool, record
    # per-step timing and complete exception detail. Costs a few seconds. Safe to leave on.
    [switch] $Diagnostic,

    # READ-ONLY investigation run. Implies -DryRun and -NoReboot. Changes NOTHING on the
    # device: it only collects the diagnostic evidence needed to find out why a fleet
    # machine fails. This is what to deploy to the Italian laptops first.
    [switch] $DiagnosticOnly,

    # Suppress the forced restart at the end of the script. IMPORTANT for fleet debugging:
...
# A read-only diagnostic run must never change anything and must never reboot.
if ($DiagnosticOnly) {
    $DryRun     = $true
    $NoReboot   = $true
    $Diagnostic = $true
...
# StrictMode Latest turns any uninitialised variable or missing property into a terminating
# error. That is good discipline, but on a fleet it can abort a run on one odd machine, so
# every step is now wrapped by Invoke-Step and a global trap records anything that escapes.
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Continue'
$ProgressPreference    = 'SilentlyContinue'

# Last-resort capture: anything that would otherwise kill the script silently is written to
# the heartbeat file, which survives even if logging was never initialised.
trap {
    try {
        $msg = "UNHANDLED: $($_.Exception.GetType().FullName): $($_.Exception.Message) at line $($_.InvocationInfo.ScriptLineNumber) [$($script:CurrentStep)]"
        if (Get-Command Write-Heartbeat -ErrorAction SilentlyContinue) { Write-Heartbeat $msg }
        else {
            Add-Content -LiteralPath 'C:\Windows\Temp\PostMigrationCleanup-heartbeat.log' -Value ('[{0}] {1}' -f (Get-Date -Format 's'), $msg) -ErrorAction SilentlyContinue
        }
        if (Get-Command Write-ErrorDetail -ErrorAction SilentlyContinue) { Write-ErrorDetail -Step "unhandled:$($script:CurrentStep)" -ErrorRecord $_ }
    } catch { }
    continue
}
...
$script:RawCaptures        = [ordered]@{}                       # verbatim external tool output
...
function Initialize-Logging {
    ...
    # ---------------------------------------------------------------------------------------
    #  Full PowerShell transcript. Captures everything - including output from commands that
    #  bypass Write-Log, and any terminating error text - which is what you need when the
    #  script fails on a machine you cannot log on to.
    # ---------------------------------------------------------------------------------------
    $script:TranscriptFile = Join-Path $script:LogDir ("Transcript-{0}-{1}-{2}.log" -f $env:COMPUTERNAME, $phase, $stamp)
    try {
        Start-Transcript -LiteralPath $script:TranscriptFile -Force -ErrorAction Stop | Out-Null
        $script:TranscriptStarted = $true
    }
    catch {
        # Non-fatal: transcript is a bonus, the normal log still works.
        $script:TranscriptFile = "not started: $($_.Exception.Message)"
    }
}
...
# =============================================================================================
#  DIAGNOSTICS MODULE
#  Added to find out why the script fails on fleet machines (e.g. the Italian laptops).
#  Everything here is read-only telemetry: it never changes device state.
# =============================================================================================

function Write-Heartbeat {
    <#
        Written before anything else can fail, to a path that always exists. If this file is
        absent on a machine that "did nothing", the script never started at all - which points
        at delivery, execution policy or antivirus rather than at the script's logic.
    #>
    param([string] $Message)
    try {
        $line = '[{0}] [{1}] {2}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'), $env:COMPUTERNAME, $Message
        Add-Content -LiteralPath $script:EarlyLogPath -Value $line -Encoding UTF8 -ErrorAction SilentlyContinue
    }
    catch { }
}

function Write-Diag {
    <# Diagnostic detail. Always written to the log file; echoed to console in -Diagnostic. #>
    param([string] $Message)
...
function Write-ErrorDetail {
    <#
        Records EVERYTHING about a failure. A one-line message is useless when triaging
        100 machines; this captures the exception type, HResult, Win32 error, the exact
        line of script that failed, and the inner exception chain.
    #>
    param(
    ...
    try {
        if ($ErrorRecord -is [System.Management.Automation.ErrorRecord]) {
            ...
            $detail.Message       = [string]$ErrorRecord.Exception.Message
            $detail.ExceptionType = $ex.GetType().FullName
            try { $detail.HResult = '0x{0:X8}' -f $ex.HResult } catch { }
            if ($ex -is [System.ComponentModel.Win32Exception]) {
                $detail.Win32Error = [string]$ex.NativeErrorCode
            }
            try { $detail.CategoryInfo = [string]$ErrorRecord.CategoryInfo } catch { }
            try { if ($ErrorRecord.TargetObject) { $detail.TargetObject = [string]$ErrorRecord.TargetObject } } catch { }
            try {
                $detail.ScriptLine = ([string]$ErrorRecord.InvocationInfo.Line).Trim()
                $detail.LineNumber = [string]$ErrorRecord.InvocationInfo.ScriptLineNumber
            } catch { }
            try { $detail.StackTrace = [string]$ErrorRecord.ScriptStackTrace } catch { }

            $chain = @()
            $inner = $ex.InnerException
            $guard = 0
            while ($inner -and $guard -lt 5) {
                $chain += ('{0}: {1}' -f $inner.GetType().Name, $inner.Message)
                $inner = $inner.InnerException
                $guard++
            }
            $detail.InnerChain = ($chain -join ' <- ')
        }
        else {
            $detail.Message = [string]$ErrorRecord
        }
    }
    catch { $detail.Message = 'Could not fully parse the error record.' }

    [void]$script:Failures.Add([pscustomobject]$detail)

    Write-Log "FAILURE in step '$Step'$(if($Context){" ($Context)"}) : $($detail.Message)" 'ERROR'
    Write-Diag "  exception   : $($detail.ExceptionType)  HResult $($detail.HResult) $(if($detail.Win32Error){"win32=$($detail.Win32Error)"})"
    if ($detail.LineNumber) { Write-Diag "  at line     : $($detail.LineNumber)  ->  $($detail.ScriptLine)" }
    if ($detail.CategoryInfo) { Write-Diag "  category    : $($detail.CategoryInfo)" }
    if ($detail.InnerChain) { Write-Diag "  inner       : $($detail.InnerChain)" }
    if ($detail.TargetObject) { Write-Diag "  target      : $($detail.TargetObject)" }
}

function Invoke-Step {
    <#
        Runs one unit of work, timed, and never lets an exception kill the whole run.
        Every step's outcome lands in the receipt, so across 100 machines you can see
        exactly which step fails and how often.
    #>
    param(
    ...
    try {
        ...
        $status = 'FAILED'
        Write-ErrorDetail -Step $Name -ErrorRecord $_ -Context $Context
    }
    finally {
        $sw.Stop()
...
function Invoke-CaptureTool {
    <#
        Runs an external tool and stores its RAW output verbatim.
        This is the single most useful thing for the Italian-laptop problem: tools like
        cmdkey print LOCALISED output, so a parser written against English text finds
        nothing and the script reports "success" having done nothing. Capturing the real
        text tells us immediately whether that is what is happening.
    #>
    param(
    ...
    if (-not (Test-Path -LiteralPath $FilePath)) {
        $script:RawCaptures[$Key] = "TOOL NOT PRESENT: $FilePath"
        Write-Diag "RAW[$Key] tool not present: $FilePath"
        return @()
    }
    ...
    try {
        ...
        if ($Redact) {
            # Keep structure, hide values that could be secrets or personal data.
            $text = $text -replace '(?i)(password|pwd|secret|token)\s*[:=]\s*\S+', '$1: <redacted>'
        }
        $script:RawCaptures[$Key] = $text
        Write-Diag "RAW[$Key] $FilePath $($Arguments -join ' ')  -> $($out.Count) line(s), exit $LASTEXITCODE"
        if ($Diagnostic) {
            foreach ($l in $out) { Write-Diag "    | $l" }
        }
    }
    catch {
        $script:RawCaptures[$Key] = "CAPTURE FAILED: $($_.Exception.Message)"
        Write-ErrorDetail -Step "capture:$Key" -ErrorRecord $_ -Context $FilePath
    }
    return $out
}
...
function Get-EnvironmentSnapshot {
    <#
        Everything about this machine that could plausibly explain a difference in behaviour
        between a working device and a failing one. Locale is captured explicitly because
        the failures reported are on Italian-language machines.
    #>

    Write-Log '--- Environment snapshot ---' 'STEP'
    ...
    try {
        ...
        $d['RunAsUser']    = $id.Name
        $d['RunAsSid']     = $id.User.Value
        $d['IsSystem']     = ($id.User.Value -eq 'S-1-5-18')
        $pr = New-Object System.Security.Principal.WindowsPrincipal($id)
        $d['IsAdmin']      = $pr.IsInRole([System.Security.Principal.WindowsBuiltInRole]::Administrator)
    } catch { $d['RunAsUser'] = "ERROR: $($_.Exception.Message)" }

    # --- PowerShell / .NET ---
    $d['PSVersion']        = $PSVersionTable.PSVersion.ToString()
    try { $d['PSEdition']  = $PSVersionTable.PSEdition } catch { }
    try { $d['CLRVersion'] = [string][System.Environment]::Version } catch { }
    $d['Is64BitProcess']   = [Environment]::Is64BitProcess
    $d['Is64BitOS']        = [Environment]::Is64BitOperatingSystem
    $d['PSHost']           = try { $Host.Name } catch { 'unknown' }
    try { $d['ExecutionPolicy'] = (Get-ExecutionPolicy -Scope Process).ToString() + ' (process) / ' + (Get-ExecutionPolicy).ToString() + ' (effective)' } catch { }
    try { $d['LanguageMode'] = $ExecutionContext.SessionState.LanguageMode.ToString() } catch { }
    try { $d['ConstrainedLanguage'] = ($ExecutionContext.SessionState.LanguageMode -ne 'FullLanguage') } catch { }

    # --- LOCALE: the prime suspect for the Italian fleet ---
    try {
        $d['UICulture']        = (Get-UICulture).Name
        $d['Culture']          = (Get-Culture).Name
        $d['CultureDisplay']   = (Get-Culture).DisplayName
        $d['SystemLocale']     = try { (Get-WinSystemLocale).Name } catch { 'n/a' }
        $d['OSLanguage']       = try { (Get-CimInstance Win32_OperatingSystem -ErrorAction Stop).OSLanguage } catch { 'n/a' }
        $d['MUILanguages']     = try { ((Get-WinUserLanguageList | Select-Object -ExpandProperty LanguageTag) -join ',') } catch { 'n/a' }
    } catch { $d['UICulture'] = "ERROR: $($_.Exception.Message)" }

    # --- OS ---
    try {
        $os = Get-CimInstance Win32_OperatingSystem -ErrorAction Stop
        $d['OSCaption']     = $os.Caption
        $d['OSVersion']     = $os.Version
        $d['OSBuild']       = $os.BuildNumber
        $d['OSArchitecture']= $os.OSArchitecture
        $d['LastBootUpTime']= [string]$os.LastBootUpTime
        $d['FreeMemoryMB']  = [int]($os.FreePhysicalMemory / 1024)
    } catch { $d['OSCaption'] = "CIM FAILED: $($_.Exception.Message)" }
    try { $d['UBR'] = (Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion' -Name UBR -ErrorAction Stop).UBR } catch { }
    try { $d['DisplayVersion'] = (Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion' -Name DisplayVersion -ErrorAction Stop).DisplayVersion } catch { }
    ...
    try {
        ...
        $d['Model']        = $cs.Model
        $d['DomainJoined'] = $cs.PartOfDomain
        $d['Domain']       = $cs.Domain
    } catch { }

    # --- disk space on the system drive (a full disk breaks deletion AND logging) ---
    try {
        $drv = Get-PSDrive -Name ($env:SystemDrive.TrimEnd(':')) -ErrorAction Stop
        $d['SystemDriveFreeGB'] = [math]::Round($drv.Free / 1GB, 2)
    } catch { }

    # --- security products that commonly block scripted deletion ---
    try {
        $av = Get-CimInstance -Namespace 'root\SecurityCenter2' -ClassName AntiVirusProduct -ErrorAction Stop
        $d['AntiVirus'] = (($av | ForEach-Object { $_.displayName }) -join ', ')
    } catch { $d['AntiVirus'] = 'not enumerable (normal on Server / when running as SYSTEM)' }
    try {
        $mp = Get-MpComputerStatus -ErrorAction Stop
        $d['DefenderRealTime'] = $mp.RealTimeProtectionEnabled
        $d['DefenderTamper']   = $mp.IsTamperProtected
    } catch { }

    # --- Office / Outlook presence: no Office means most steps legitimately no-op ---
    try {
        $c2r = Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration' -ErrorAction Stop
        $d['OfficeC2RVersion']  = $c2r.VersionToReport
        $d['OfficeC2RProducts'] = $c2r.ProductReleaseIds
        $d['OfficeUILang']      = try { $c2r.ClientCulture } catch { 'n/a' }
    } catch { $d['OfficeC2RVersion'] = 'Click-to-Run not detected' }
    foreach ($exe in @('OUTLOOK.EXE','WINWORD.EXE','EXCEL.EXE')) {
        try {
            $p = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\$exe" -ErrorAction Stop).'(default)'
            $d["Installed_$exe"] = $p
        } catch { $d["Installed_$exe"] = 'not registered' }
    }

    foreach ($k in $d.Keys) { Write-Log ("  {0,-22}: {1}" -f $k, $d[$k]) }
    return $d
}

function Test-Prerequisites {
    <#
        Confirms every external dependency the script relies on is actually present and
        callable. If one of these is missing or blocked, the script silently degrades -
        which is exactly the "it doesn't work but reports success" symptom.
    #>
    Write-Log '--- Prerequisite checks ---' 'STEP'

    $tools = [ordered]@{
        'reg.exe'       = (Join-Path $env:SystemRoot 'System32\reg.exe')
        'cmdkey.exe'    = (Join-Path $env:SystemRoot 'System32\cmdkey.exe')
        'dsregcmd.exe'  = (Join-Path $env:SystemRoot 'System32\dsregcmd.exe')
        'robocopy.exe'  = (Join-Path $env:SystemRoot 'System32\robocopy.exe')
        'powershell.exe'= (Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe')
    }
    foreach ($t in $tools.Keys) {
        $present = Test-Path -LiteralPath $tools[$t]
        $script:Diag["Tool_$t"] = if ($present) { $tools[$t] } else { 'MISSING' }
        if ($present) { Write-Log "  $t : present" 'OK' } else { Write-Log "  $t : MISSING - dependent steps will fail" 'ERROR' }
    }

    # Modules used for the user-session phase.
    foreach ($m in @('ScheduledTasks','CimCmdlets')) {
        $ok = $null -ne (Get-Module -ListAvailable -Name $m -ErrorAction SilentlyContinue)
        $script:Diag["Module_$m"] = $ok
        if ($ok) { Write-Log "  module $m : available" 'OK' } else { Write-Log "  module $m : NOT available - user phase cannot run" 'WARN' }
    }

    # Can we actually write where we intend to?
    foreach ($p in @($script:LogDir, $script:WorkRoot)) {
        try {
            if (-not (Test-Path -LiteralPath $p)) { New-Item -ItemType Directory -Path $p -Force -ErrorAction Stop | Out-Null }
            $probe = Join-Path $p ('.probe-' + [guid]::NewGuid().ToString('N'))
            Set-Content -LiteralPath $probe -Value 'x' -ErrorAction Stop
            Remove-Item -LiteralPath $probe -Force -ErrorAction SilentlyContinue
            Write-Log "  writable: $p" 'OK'
            $script:Diag["Writable_$p"] = $true
        }
        catch {
            Write-Log "  NOT writable: $p - $($_.Exception.Message)" 'ERROR'
            $script:Diag["Writable_$p"] = $false
        }
    }

    # WMI/CIM health - the script depends on it to find the signed-in user.
    try {
        $null = Get-CimInstance Win32_OperatingSystem -ErrorAction Stop
        Write-Log '  CIM/WMI : responding' 'OK'
        $script:Diag['CimHealthy'] = $true
    }
    catch {
        Write-Log "  CIM/WMI : FAILING - $($_.Exception.Message)" 'ERROR'
        Write-Log '  Without WMI the script cannot detect the signed-in user (exit code 3).' 'ERROR'
        $script:Diag['CimHealthy'] = $false
    }
}

function Get-ProfileDiagnostics {
    <#
        Dumps the raw ProfileList and the explorer.exe owner detection, because
        "No eligible user profile found" (exit 3) is the most likely fleet failure and
        this shows exactly why the filter rejected every profile.
    #>
    Write-Log '--- User profile enumeration (raw) ---' 'STEP'

    $listKey = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList'
    try {
        $entries = @(Get-ChildItem -LiteralPath $listKey -ErrorAction Stop)
        Write-Log "  ProfileList entries: $($entries.Count)"
        foreach ($e in $entries) {
            $sid  = Split-Path $e.PSPath -Leaf
            $path = ''
            try { $path = (Get-ItemProperty -LiteralPath $e.PSPath -Name ProfileImagePath -ErrorAction Stop).ProfileImagePath } catch { $path = '<unreadable>' }

            $verdict = 'ACCEPTED'
            if ($sid -notmatch '^S-1-5-21-\d+-\d+-\d+-\d+$') { $verdict = 'rejected: not a user SID pattern' }
            else {
                $parts = $sid -split '-'
                $rid = 0
                if (-not [int]::TryParse($parts[$parts.Count-1], [ref]$rid)) { $verdict = 'rejected: RID unparseable' }
                elseif ($rid -lt 500) { $verdict = "rejected: RID $rid below 500" }
                elseif (-not (Test-Path -LiteralPath $path)) { $verdict = 'rejected: profile path does not exist on disk' }
                else {
                    $leaf = Split-Path $path -Leaf
                    if ($leaf -match '^(Default|Default User|Public|All Users|systemprofile|LocalService|NetworkService|TEMP)$') { $verdict = "rejected: built-in profile '$leaf'" }
                    elseif ($path -match '(?i)\\config\\systemprofile') { $verdict = 'rejected: system profile' }
                }
            }
            Write-Log ("    {0}  {1}  -> {2}" -f $sid, $path, $verdict)
            Write-Diag ("    appdata-local exists: {0}" -f (Test-Path -LiteralPath (Join-Path $path 'AppData\Local')))
        }
    }
    catch {
        Write-ErrorDetail -Step 'profile-enumeration' -ErrorRecord $_ -Context $listKey
    }

    # Who is actually signed in?
    Write-Log '  Interactive session detection (explorer.exe owners):'
    try {
        $procs = @(Get-CimInstance -ClassName Win32_Process -Filter "Name = 'explorer.exe'" -ErrorAction Stop)
        if ($procs.Count -eq 0) {
            Write-Log '    no explorer.exe running - NOBODY is signed in interactively.' 'WARN'
            Write-Log '    On a fleet run at startup this alone produces exit code 3.' 'WARN'
        }
        foreach ($p in $procs) {
            try {
                $o = Invoke-CimMethod -InputObject $p -MethodName GetOwner -ErrorAction Stop
                Write-Log ("    pid {0} owned by {1}\{2}" -f $p.ProcessId, $o.Domain, $o.User)
            }
            catch { Write-Log "    pid $($p.ProcessId): GetOwner failed - $($_.Exception.Message)" 'WARN' }
        }
    }
    catch {
        Write-ErrorDetail -Step 'session-detection' -ErrorRecord $_ -Context 'Win32_Process explorer.exe'
    }

    # Quser gives the session table including disconnected/RDP sessions.
    $q = Join-Path $env:SystemRoot 'System32\quser.exe'
    if (Test-Path -LiteralPath $q) { [void](Invoke-CaptureTool -Key 'quser' -FilePath $q) }
}

function Get-LocalizationDiagnostics {
    <#
        THE key check for the Italian fleet. The script parses the text output of cmdkey
        and dsregcmd. dsregcmd field names are not localised, but cmdkey's ARE. If the
        credential parser matches nothing here while the machine clearly has cached
        Microsoft credentials, we have found the fault.
    #>
    Write-Log '--- Localisation / external tool output check ---' 'STEP'

    $cmdkey = Join-Path $env:SystemRoot 'System32\cmdkey.exe'
    $raw = @(Invoke-CaptureTool -Key 'cmdkey_list' -FilePath $cmdkey -Arguments @('/list') -Redact)

    if ($raw.Count -eq 0) {
        Write-Log '  cmdkey returned no output at all.' 'WARN'
    }
    else {
        # Re-run the script's own parser and report what it managed to extract.
        ...
        foreach ($line in $raw) {
            ...
            if ($text -match '(?i)((?:LegacyGeneric|WindowsLive|MicrosoftAccount|Domain|virtualapp)\s*:\s*\S.*)$') {
                [void]$matched.Add($Matches[1].Trim())
            }
        }
        Write-Log "  cmdkey lines returned      : $($raw.Count)"
        Write-Log "  targets the parser matched : $($matched.Count)"
        $script:Diag['CmdkeyLines']   = $raw.Count
        $script:Diag['CmdkeyMatched'] = $matched.Count

        if ($matched.Count -eq 0 -and $raw.Count -gt 3) {
            Write-Log '  *** PARSER MATCHED NOTHING although cmdkey produced output. ***' 'ERROR'
            Write-Log '  *** This is very likely the fleet failure: the credential parser is ***' 'ERROR'
            Write-Log '  *** not recognising this locale output. Raw text is in the receipt. ***' 'ERROR'
            $script:Diag['SuspectedLocaleParsingFailure'] = $true
        }
        foreach ($m in $matched) { Write-Diag "    matched target: $m" }
    }

    # dsregcmd - field names are locale-independent, but capture it to be certain.
    $dsreg = Join-Path $env:SystemRoot 'System32\dsregcmd.exe'
    [void](Invoke-CaptureTool -Key 'dsregcmd_status' -FilePath $dsreg -Arguments @('/status'))

    # Known-folder locations: confirms the real on-disk names on a localised Windows
    # (they should stay English even when Explorer shows Italian names).
    try {
        foreach ($f in @('Personal','Desktop','My Pictures','Local AppData','AppData')) {
            try {
                $v = (Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders' -Name $f -ErrorAction Stop).$f
                Write-Diag "  shell folder '$f' = $v"
                $script:Diag["ShellFolder_$($f -replace '\s','')"] = $v
            } catch { }
        }
    } catch { }
}
...
function Write-Summary {
    ...
    # --- diagnostic verdict: the part a technician reads first ---
    ...
    if ($failed.Count -gt 0) {
        ...
        foreach ($f in $failed) {
            Write-Log ("  {0} [{1}] after {2} ms" -f $f.Step, $f.Context, $f.DurationMs) 'ERROR'
        }
        foreach ($f in $script:Failures) {
            Write-Log ("  {0}: {1} ({2}) line {3}" -f $f.Step, $f.Message, $f.ExceptionType, $f.LineNumber) 'ERROR'
        }
    }
    if ($script:Diag.Contains('SuspectedLocaleParsingFailure')) {
        Write-Log '*** LIKELY ROOT CAUSE: credential parsing produced no matches on this locale. ***' 'ERROR'
        Write-Log '*** Send the receipt JSON - RawToolOutput.cmdkey_list has the actual text.    ***' 'ERROR'
    }
    ...
    try {
        $receipt = [pscustomobject]@{
            ...
            # --- diagnostic payload: this is what makes 100 machines comparable ---
            DiagVersion     = $script:DiagVersion
            Environment     = $script:Diag
            Steps           = @($script:Steps)
        ...
        $receipt | ConvertTo-Json -Depth 8 | Out-File -LiteralPath $file -Encoding UTF8 -Force
...
# Heartbeat FIRST, before anything that can fail. If this line never appears on a machine,
# the script never started - look at delivery / execution policy / antivirus, not the logic.
Write-Heartbeat "START pid=$PID user=$env:USERNAME phase=$(if($UserPhase){'user'}else{'system'}) args=[$($MyInvocation.Line)]"

Initialize-Logging

Write-Log '=================================================================' 'STEP'
Write-Log " Post-Migration Cleanup  v$($script:Version)  (diag $($script:DiagVersion))" 'STEP'
Write-Log ' Microsoft 365 tenant-to-tenant - cache, token and session reset' 'STEP'
Write-Log '=================================================================' 'STEP'
Write-Log ("Log file         : {0}" -f $script:LogFile)
Write-Log ("Transcript       : {0}" -f $script:TranscriptFile)
if ($DiagnosticOnly) {
    Write-Log '*** DIAGNOSTIC-ONLY RUN: read-only. Nothing on this device will change. ***' 'STEP'
}

# --- Diagnostics: environment, prerequisites, profiles, localisation -----------------------
Invoke-Step -Name 'diag:environment'   -Action { $null = Get-EnvironmentSnapshot }
Invoke-Step -Name 'diag:prerequisites' -Action { Test-Prerequisites }
Invoke-Step -Name 'diag:profiles'      -Action { Get-ProfileDiagnostics }
Invoke-Step -Name 'diag:localisation'  -Action { Get-LocalizationDiagnostics }

if ($DiagnosticOnly) {
    Write-Log '--- Diagnostic-only run complete: skipping all cleanup actions ---' 'STEP'
    Write-Summary -ProcessedUsers 0
    Write-Heartbeat 'END diagnostic-only'
...
try {
    ...
    foreach ($user in $users) {
        ...
        Invoke-Step -Name 'outlook-datafile' -Context $u -Action { Invoke-OutlookDataFileCleanup -User $user }
        Invoke-Step -Name 'outlook-profile'  -Context $u -Action { Invoke-OutlookProfileReset    -User $user -HiveRoot $hiveRoot }
        Invoke-Step -Name 'office-identity'  -Context $u -Action { Invoke-OfficeIdentityReset    -User $user -HiveRoot $hiveRoot }
        Invoke-Step -Name 'onedrive-unlink'  -Context $u -Action { Invoke-OneDriveUnlink         -User $user -HiveRoot $hiveRoot }
        Invoke-Step -Name 'wam-accounts'     -Context $u -Action { Invoke-WamAccountCleanup      -User $user -HiveRoot $hiveRoot }
...
Write-Heartbeat "END exit=$exitCode errors=$($script:ErrorCount) warnings=$($script:WarningCount) failedSteps=$(@($script:Steps | Where-Object { $_.Status -eq 'FAILED' }).Count)"

# Close the transcript BEFORE any restart, otherwise the file is left incomplete.
